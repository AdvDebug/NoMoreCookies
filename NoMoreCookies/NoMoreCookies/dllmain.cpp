...
BOOL IsProcessAllowed()
{
    ...
    if (XMode && IsSigned(GetCurrentProcess()))
    {
        if (hasEnding(FileName, "msedge.exe") || hasEnding(FileName, "firefox.exe") || hasEnding(FileName, "chrome.exe") || hasEnding(FileName, "brave.exe"))
            return true;
    }
